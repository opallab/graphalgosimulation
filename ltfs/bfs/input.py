...
def build_input(n: int, start: int):
    ...
    hstack["order_i"] = torch.zeros(rows, 1)
    ...
    # binary flags
    hstack["bin_disc"] = torch.zeros(rows, 1)
    hstack["bin_visit"] = torch.zeros(rows, 1)
    hstack["bin_visitn"] = torch.zeros(rows, 1)
    ...
    # scratchpad
    hstack["S"] = torch.zeros(rows, 50)
    hstack["S_change"] = torch.zeros(rows, 1)
    hstack["S_bin_switch"] = torch.zeros(rows, 1)
    hstack["S_bin_term_1"] = torch.zeros(rows, 1)
    hstack["S_bin_term_2"] = torch.zeros(rows, 1)
