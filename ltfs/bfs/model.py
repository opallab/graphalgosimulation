...
class Model(BaseLayer):
    ...
    def forward_step(self, X, A, step: str = None):
            ...
            X = self._read(X, A)
            if step == "read" or step == "mask_write":
                return X
    ...
    def _build_model(self, d: int, m: int, index: dict, grad: bool):
        ...
        self._write = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index,
                ["M_P_best", "M_P_int_best", "M_val_best"],
                ["P_i", "P_i_int", "order_i"], "bin_write",
                ["P_i", "P_i_int", "order_i"], grad=grad)[0]
        )
        ...
        # step 22
        self._interrupt_1 = BaseTransformer.from_pretrained(
            empty_attention(),
            layers.interrupt_1(d, index, grad=grad),
        )
        ...
        # step 25
        self._interrupt_2 = BaseTransformer.from_pretrained(
            empty_attention(),
            layers.interrupt_2(d, index, grad=grad),
        )
