...
def build_input(n: int, start: int):
    ...
    hstack["A_is_zero"] = torch.zeros(rows, 1)
    ...
    # scratchpad
    hstack["S"] = torch.zeros(rows, 33)
    hstack["S_is_less"] = torch.zeros(rows, 1)
    hstack["S_inf"] = torch.zeros(rows, 1)
