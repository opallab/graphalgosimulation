...
from ltfs.common.proto import BaseLayer, BaseTransformer, BaseMLP, BaseAttention
...
class Model(BaseLayer):
    ...
    def _build_model(self, d: int, m: int, index: dict, grad: bool):
        ...
        self._write = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index,
                ["M_val_best", "M_P_best", "M_P_int_best"],
                ["D_i", "P_i", "P_i_int"], "bin_write",
                ["D_i", "P_i", "P_i_int"], grad=grad)[0]
        )
        ...
        # step 17
        self._mark_zeros = BaseTransformer.from_pretrained(
            empty_attention(),
            layers.mark_zeros(d, index, grad=grad),
        )
        ...
        # step 19
        self._less_than = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.less_than(d, index, "A_row", "D", "S_is_less", grad=grad)[0],
        )
        ...
        self._if_else = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index, ["A_row", "P_n"],
                ["D", "OUT"], "S_is_less", ["D", "OUT"], grad=grad)[0]
        )
        ...
        self._keep_termination = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index, ["TERM"],
                ["S_bin_term"], "TERM", ["TERM"], grad=grad)[0]
        )
